...
class UserService:
    ...
    async def sync_user_from_feishu(self, user_info: Dict) -> User:
        ...
        # 查找现有用户（使用异步查询）
        result = await self.db.execute(select(User).filter(User.feishu_user_id == feishu_user_id))
    ...
    async def list_users_simple(
        ...
        Args:
            skip: 跳过数量
            limit: 限制数量
            search: 搜索关键词（用户名或飞书ID）

        Returns:
            (用户列表, 总数)
        """
        # 构建查询 - 只查询需要的字段
        stmt = select(User.feishu_user_id, User.username).where(User.status == "active")
        count_stmt = select(func.count(User.id)).where(User.status == "active")

        # 搜索条件
        if search:
            search_filter = or_(
                User.username.ilike(f"%{search}%"), User.feishu_user_id.ilike(f"%{search}%")
            )
            stmt = stmt.where(search_filter)
            count_stmt = count_stmt.where(search_filter)
