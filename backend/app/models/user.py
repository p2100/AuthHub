...
class User(Base):
    ...
    resource_bindings = relationship(
        "ResourceBinding", back_populates="user", foreign_keys="[ResourceBinding.user_id]"
    )
    created_user_roles = relationship(
        "UserRole", back_populates="creator", foreign_keys="[UserRole.created_by]"
    )
